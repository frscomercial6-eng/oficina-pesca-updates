...
# -*- coding: utf-8 -*-
"""
Bloco de inicializaÃ§Ã£o blindado para PyInstaller e execuÃ§Ã£o local.
Este bloco garante que sys.path e diretÃ³rios estejam corretos ANTES de qualquer importaÃ§Ã£o de mÃ³dulos locais ou de terceiros.
...
import urllib.request   # <--- Garante as requisiÃ§Ãµes de atualizaÃ§Ã£o
import urllib.error
import subprocess
import atexit
import threading
...
# ============================================================================== 
# CONFIGURAÃ‡ÃƒO DE DIRETÃ“RIO DO PYINSTALLER (MantÃ©m o que jÃ¡ estÃ¡ funcionando)
# ==============================================================================
...
# IMPORTS DOS SEUS MÃ“DULOS LOCAIS (Que o Mestre jÃ¡ resolveu!)
...
def _controlar_falhas_inicio() -> None:
    ...
    if int(estado.get("consecutivas", 0)) >= 3:
        logger.warning("[startup] Detectadas 3 falhas seguidas; executando limpeza automÃ¡tica de cache.")
...
def _garantir_instancia_unica() -> tuple[bool, str]:
    ...
    if os.path.exists(_STARTUP_LOCK_PATH):
        try:
            ...
            if pid_antigo and pid_antigo != atual and _processo_existe(pid_antigo):
                return False, "JÃ¡ existe outra instÃ¢ncia do sistema em execuÃ§Ã£o."
            os.remove(_STARTUP_LOCK_PATH)
        except Exception:
            pass
    ...
    with open(_STARTUP_LOCK_PATH, "w", encoding="utf-8") as f:
        f.write(str(atual))
    return True, "Trava de instÃ¢ncia Ãºnica aplicada."


def _varrer_config_corrompido() -> tuple[bool, str]:
    cfg_path = os.path.join(_base_runtime_dir(), "config.cfg")
    if not os.path.exists(cfg_path):
        return False, "config.cfg nÃ£o encontrado; nenhuma correÃ§Ã£o necessÃ¡ria."
    ...
    try:
        ...
        try:
            ...
            return False, "config.cfg invÃ¡lido e nÃ£o foi possÃ­vel renomear para backup."
    ...
    if not parser.has_section("app"):
        ...
        alterado = True
    if not parser.has_option("app", "modo"):
        parser.set("app", "modo", "local")
        alterado = True
    if not parser.has_option("app", "servidor_url"):
        parser.set("app", "servidor_url", "http://localhost:8000")
        alterado = True

    if not parser.has_section("suporte"):
        parser.add_section("suporte")
        alterado = True
    if not parser.has_option("suporte", "envio_logs_quinzenal"):
        parser.set("suporte", "envio_logs_quinzenal", "true")
        alterado = True
    if not parser.has_option("suporte", "log_upload_intervalo_dias"):
        parser.set("suporte", "log_upload_intervalo_dias", "15")
        alterado = True

    if alterado:
        with open(cfg_path, "w", encoding="utf-8") as f:
            parser.write(f)
        return True, "Varredura de configuraÃ§Ã£o concluÃ­da e chaves obrigatÃ³rias normalizadas."
    return True, "Varredura de configuraÃ§Ã£o concluÃ­da sem necessidade de ajuste."
...
def _executar_varredura_inicializacao() -> None:
    ...
    logger.info("[startup] VerificaÃ§Ã£o de instÃ¢ncia: %s", msg_instancia)
    if not ok_instancia:
    ...
    if ok_cfg:
        logger.info("[startup] ConfiguraÃ§Ã£o: %s", msg_cfg)
    else:
        logger.warning("[startup] ConfiguraÃ§Ã£o: %s", msg_cfg)
...
def _inicializar_fluxo_pos_termos() -> None:
    ...
    logger.info("[startup] Inicializando conexÃ£o com banco de dados.")
    inicializar_banco()
    ...
    # Envio automÃ¡tico de logs removido da inicializaÃ§Ã£o para evitar uso de rede.
    ...
    # VerificaÃ§Ã£o de versÃ£o desativada na inicializaÃ§Ã£o automÃ¡tica para evitar uso de rede.
...
def abrir_janela_planos(dias_restantes_alerta: int | None = None):
    try:
        ...
        messagebox.showerror(
            "Planos",
            "NÃ£o foi possÃ­vel abrir a tela de planos no momento.",
            parent=janela_login,
        )
...
def _obter_cfg_pagamento_runtime() -> tuple[str, str, str, str, str]:
    """RelÃª config.cfg em tempo real para nÃ£o exigir reinÃ­cio ao alterar pagamento."""
    try:
        ...
        if not cfg_path_usado:
            logger.info("config.cfg nÃ£o encontrado em runtime nos caminhos esperados de pagamento.")
...
def _enviar_mensagem_pagamento_whatsapp_admin(tipo_pagamento: str):
    ...
    if not numero_admin:
        messagebox.showwarning(
            "WhatsApp",
            "NÃºmero do administrador nÃ£o configurado em config.cfg (app.whatsapp_admin).",
            parent=janela_login,
        )
        return
    ...
    texto = (
        f"OlÃ¡, sou {usuario}. Acabei de realizar o pagamento ({tipo_pagamento}) no sistema Oficina de Pesca. "
    ...
    messagebox.showwarning(
        "WhatsApp",
        "NÃ£o foi possÃ­vel abrir o WhatsApp automaticamente.",
        parent=janela_login,
    )
...
def _criar_link_checkout_infinitepay(
    ...
    descricao: str,
    referencia: str = "",
    item_descricao: str = "AtualizaÃ§Ã£o",
) -> str:
    """Tenta criar checkout dinÃ¢mico na InfinitePay e retorna URL vazia em falha."""
    ...
    if not token_cfg:
        if debug_ativo:
            logger.info(
                "InfinitePay debug (primeira tentativa): Token nÃ£o configurado em config.cfg (pagamento.infinitepay_api_token)."
            )
        return ""
    ...
    item_descricao = str(item_descricao or "AtualizaÃ§Ã£o").strip() or "AtualizaÃ§Ã£o"
...
def _abrir_link_infinitepay_se_configurado(
    ...
    if valor_reais > 0:
        link_pagamento = _criar_link_checkout_infinitepay(
            valor_reais,
            descricao or "AtualizaÃ§Ã£o de versÃ£o",
            referencia,
            item_descricao,
        )
    ...
    if not link_pagamento:
        messagebox.showwarning(
            "InfinitePay",
            "NÃ£o foi possÃ­vel obter o link de pagamento.\n\n"
            ...
            "ou\n"
            "- pagamento.infinitepay_api_token (para geraÃ§Ã£o automÃ¡tica)",
            parent=janela_login,
        )
        return
    ...
    messagebox.showwarning(
        "InfinitePay",
        "NÃ£o foi possÃ­vel abrir o link automaticamente.\n\n"
        f"Link: {link_pagamento}\n\n"
        "O link foi copiado para a Ã¡rea de transferÃªncia (quando disponÃ­vel).",
        parent=janela_login,
    )
...
def abrir_tela_cadastro():
    ...
    def salvar():
        ...
        if u and s and sc:
            if s != sc:
                label_status.configure(text="âŒ Senhas nÃ£o coincidem!", text_color="red")
                return
            valid, mensagem = validate_password(s)
            if not valid:
                label_status.configure(text=f"âŒ {mensagem}", text_color="red")
                return
            try:
                ...
                label_status.configure(text=f"âœ… Usuário {u} criado!", text_color="green")
            except sqlite3.IntegrityError:
                label_status.configure(text="âŒ Usuário jÃ¡ existe!", text_color="red")
            except Exception:
                label_status.configure(text="âŒ Erro ao criar usuário!", text_color="red")
        else:
            label_status.configure(text="âŒ Preencha todos os campos!", text_color="red")

    ctk.CTkButton(jan_cad, text="SALVAR", fg_color="#27ae60", command=salvar).pack(pady=20)


def abrir_tela_primeiro_admin():
    # Evita abrir duplicado se jÃ¡ existe uma janela de criaÃ§Ã£o aberta
    ...
    def salvar_admin():
        ...
        if not u or not s or not sc:
            ...
            return
        if s != sc:
            lbl_local.configure(text="As senhas nÃ£o coincidem.")
            return
        ...
        try:
            ...
            atualizar_status_trial_tela()
            label_status.configure(text="âœ… ADMIN inicial criado. FaÃ§a login.", text_color="green")
            
            # Comandos de transição agora corretamente indentados dentro do bloco try
            janela_login.deiconify()
            atualizar_status_primeiro_acesso()
        except sqlite3.IntegrityError:
            lbl_local.configure(text="Usuário jÃ¡ existe.")
        except Exception as e:
...
def verificar_login():
    ...
    logger.info("[startup] Iniciando tentativa de autenticaÃ§Ã£o de usuário.")
    ...
    try:
        ...
        logger.exception("Erro ao verificar existÃªncia de usuÃ¡rios: %s", e)
        label_status.configure(text="âŒ Erro ao acessar base de usuÃ¡rios.", text_color="red")
        return

    print(f"Log: Autenticando usuário '{u}'...")

    if not u or not s:
        label_status.configure(text="âŒ Informe usuário e senha.", text_color="red")
        return
...
def abrir_tela_ativacao():
    ...
    ctk.CTkLabel(dialog, text="Chave de instalaÃ§Ã£o deste PC:", text_color="#bdc3c7").pack(pady=(18, 4))
    ...
    def _copiar_chave():
        ...
        btn_copiar.configure(text="âœ… Copiado!")
        dialog.after(2000, lambda: btn_copiar.configure(text="ðŸ“‹ Copiar Chave"))

    btn_copiar = ctk.CTkButton(frame_chave, text="ðŸ“‹ Copiar Chave", command=_copiar_chave,
                               width=130, height=30, fg_color="#2980b9", hover_color="#3498db")
    btn_copiar.pack(side="left")

    ctk.CTkLabel(dialog, text="Contra-senha de ativaÃ§Ã£o:", text_color="#bdc3c7").pack(pady=(0, 4))
    ...
    def _confirmar_ativacao():
        ...
        if not chave:
            messagebox.showwarning("AtivaÃ§Ã£o", "Informe a contra-senha de ativaÃ§Ã£o.", parent=dialog)
            return

        ok, msg = ativar_licenca(chave)
        if ok:
            messagebox.showinfo("AtivaÃ§Ã£o", msg, parent=dialog)
        ...
        try:
            ...
            if detalhe:
                msg = f"{msg}\n\nDiagnÃ³stico: {detalhe}"
        except Exception:
            pass
        messagebox.showerror("AtivaÃ§Ã£o", msg, parent=dialog)
...
ctk.CTkLabel(main_frame, text="Cadastro de usuários disponÃ­vel apenas no menu do ADMIN.", text_color="#95a5a6", wraplength=300, justify="center").pack(pady=(0, 10))
...
def _executar_instalacao_update():
    if not _url_update_disponivel:
        messagebox.showwarning("AtualizaÃ§Ã£o", "Link de atualizaÃ§Ã£o indisponÃ­vel no momento.", parent=janela_login)
        return

    confirmar = messagebox.askyesno(
        "AtualizaÃ§Ã£o",
        "Deseja baixar e instalar a nova versÃ£o agora?\n\n"
        "O sistema serÃ¡ fechado para concluir a atualizaÃ§Ã£o.",
        parent=janela_login,
    )
    if not confirmar:
        return
    ...
    pop = ctk.CTkToplevel(janela_login)
    pop.title("Atualização")
    pop.geometry("420x170")
    pop.resizable(False, False)
    pop.transient(janela_login)
    pop.grab_set()
    pop.focus_force()

    lbl_status_update = ctk.CTkLabel(pop, text="Preparando atualização...", anchor="w")
    lbl_status_update.pack(fill="x", padx=16, pady=(16, 8))

    barra_update = ctk.CTkProgressBar(pop, width=380)
    barra_update.set(0)
    barra_update.pack(padx=16, pady=(0, 8))

    lbl_percentual = ctk.CTkLabel(pop, text="0%", text_color="#aab7c4")
    lbl_percentual.pack(pady=(0, 10))

    btn_atualizar.configure(state="disabled")

    def _atualizar_ui(payload: dict):
        try:
            percent = float(payload.get("percent", 0.0) or 0.0)
            msg = str(payload.get("message", "Atualizando...") or "Atualizando...")
            barra_update.set(max(0.0, min(1.0, percent / 100.0)))
            lbl_status_update.configure(text=msg)
            lbl_percentual.configure(text=f"{int(percent)}%")
        except Exception:
            pass

    def _progresso_cb(payload: dict):
        janela_login.after(0, lambda p=payload: _atualizar_ui(p))
...
def _fluxo_pagamento_atualizacao_mensal():
    prosseguir = messagebox.askyesno(
        "Pagamento da atualizaÃ§Ã£o",
        _texto_pagamento_infinitepay("da atualizaÃ§Ã£o mensal", VALOR_ATUALIZACAO_NAO_PERMANENTE),
        parent=janela_login,
    )
    if not prosseguir:
        return
    ...
    _abrir_link_infinitepay_se_configurado(
        ...
        descricao="AtualizaÃ§Ã£o mensal Oficina de Pesca",
        referencia="ATUALIZACAO_MENSAL",
        item_descricao="AtualizaÃ§Ã£o",
    )
    _oferecer_envio_whatsapp_admin("AtualizaÃ§Ã£o")

    confirmou = messagebox.askyesno(
        "ConfirmaÃ§Ã£o de pagamento",
        "Pagamento realizado?\n\nSe sim, clique em 'Sim' para liberar a atualizaÃ§Ã£o agora.",
        parent=janela_login,
    )
    if not confirmou:
        return
...
def atualizar_status_trial_tela():
    ...
    label_status.configure(text="âŒ Trial expirado. Login bloqueado ate ativacao.", text_color="red")

def atualizar_status_primeiro_acesso():
    """Verifica se existem usuÃ¡rios cadastrados e atualiza o alerta na tela de login."""
    try:
        ...
        if usuario_count == 0:
            ...
            janela_login.withdraw()
            label_status.configure(text="âš ï¸ Crie o ADMIN inicial para continuar.", text_color="#f1c40f")
            abrir_tela_primeiro_admin()
        else:
            # Se houver usuários, garante que a tela de login apareça
            janela_login.deiconify()
            if label_status.cget("text") == "âš ï¸ Crie o ADMIN inicial para continuar.":
                label_status.configure(text="")
    except Exception as e:
        logger.error(f"Falha ao checar primeiro acesso no startup: {e}")

# â”€â”€â”€ VerificaÃ§Ã£o de nova versÃ£o (em background, nÃ£o bloqueia o login) â”€â”€â”€â”€â”€â”€â”€â”€
...
def _checar_versao_bg():
    ...
    if disponivel:
        ...
        if URL_CHECK_LICENCAS and not remoto_ok:
            ...
            msg_politica = f"Licença sem liberaÃ§Ã£o online: {msg_remota}"
        else:
        ...
        def _mostrar():
            ...
            if auto_update_liberado:
                texto_update = f"ðŸ”” Nova versÃ£o {versao_nova} disponÃ­vel ({tipo_txt}). {msg_politica}"
                cor = "#2ecc71"
                if url_download:
                    _url_update_disponivel = url_download
                    btn_atualizar.configure(
                        state="normal",
                        text="â¬† ATUALIZAR AGORA",
                        fg_color="#16a34a",
                        hover_color="#15803d",
                        border_width=2,
                        border_color="#facc15",
